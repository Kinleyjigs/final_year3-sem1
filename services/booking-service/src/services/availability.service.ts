...
export class AvailabilityService {
   ...
   */
  async getAvailability(params: AvailabilityParams): Promise<DayAvailability[]> {
    ...
    try {
      ...
      logger.info('Fetched bookings for availability', {
        groundId,
        startDate,
        endDate,
      ...
      while (currentDate <= end) {
        ...
        const dayBookings = bookings.filter(
          (b) => this.formatDate(b.bookingDate) === dateStr
        );
      ...
      logger.error('Failed to get availability', { error, params });
      throw error;
    }
  }
  ...
  private generateDayAvailability(
    ...
    for (let hour = operatingHours.start; hour < operatingHours.end; hour++) {
      ...
      const slot = this.determineSlotStatus(
        date,
        startTime,
        endTime,
        bookings,
   ...
   */
  private timeStringToMinutes(time: string | Date): number {
    let timeStr: string;
