...
class BookingService {
    constructor(prisma, groundsClient, maintenanceClient, notificationClient) {
        this.prisma = prisma;
        this.groundsClient = groundsClient;
    ...
    async createBooking(data) {
        ...
        `;
            if (conflicts.length > 0) {
                throw new common_1.ConflictError('This time slot is no longer available.');
            }
            ...
            if (this.maintenanceClient) {
                ...
                if (maintenanceConflict.hasConflict) {
                    throw new common_1.ConflictError('Ground under maintenance during selected time.');
                }
            }
    ...
    async cancelBooking(data) {
        ...
        if (!booking) {
            throw new common_1.ValidationError('Booking not found');
        }
        // 2. Verify ownership
        if (booking.userId !== data.userId) {
            throw new common_1.ValidationError('You are not authorized to cancel this booking');
        }
        // 3. Check if already canceled or rejected
        if (booking.status === BookingStatus.CANCELED) {
            throw new common_1.ValidationError('This booking is already canceled');
        }
        if (booking.status === BookingStatus.REJECTED) {
            throw new common_1.ValidationError('Cannot cancel a rejected booking');
        }
        // 4. Check if booking is in the past
        const bookingDateTime = new Date(`${booking.bookingDate.toISOString().split('T')[0]}T${booking.startTime.toISOString().split('T')[1]}`);
        if (bookingDateTime < new Date()) {
            throw new common_1.ValidationError('Cannot cancel a past booking');
        }
     ...
     */
    async getGroundBookings(groundId, date) {
...
exports.bookingService = new BookingService(prisma, undefined, // groundsClient - will be injected by gRPC server
